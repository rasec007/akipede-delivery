...
export default function CatalogoDeliveryPage() {
  ...
  // Sincroniza o carrinho se os preços ou produtos mudarem no catálogo via Pusher
  useEffect(() => {
    if (data?.categorias && cart.length > 0) {
      let changed = false;
      const newCart = cart.map(cartItem => {
        let updatedProd: any = null;
        for (const cat of data.categorias) {
          const p = cat.produto_produto_categoriaTocategoria.find((p: any) => p.id_produto === cartItem.produto_id);
          if (p) { updatedProd = p; break; }
        }

        if (!updatedProd) return cartItem;

        const baseValue = Number(updatedProd.valor_total) || 0;
        let itemsValue = 0;
        
        cartItem.itens.forEach((cartSubItem: any) => {
          let updatedItem: any = null;
          updatedProd.complemento_tipo_complemento_tipo_produtoToproduto.forEach((c: any) => {
            const found = c.complemento_item_complemento_item_complemento_tipoTocomplemento_tipo.find((i: any) => i.id_complemento_item === cartSubItem.id_item);
            if (found) updatedItem = found;
          });
          if (updatedItem) itemsValue += (Number(updatedItem.valor) || 0) * cartSubItem.quantidade;
        });

        const newUnitValue = baseValue + itemsValue;
        const newTotalValue = newUnitValue * cartItem.quantidade;

        if (cartItem.valor_unitario !== newUnitValue) {
          changed = true;
          return { ...cartItem, valor_unitario: newUnitValue, valor_total: newTotalValue };
        }
        return cartItem;
      });
  ...
  }, [data, cart.length]); // Removido 'cart' das dependências para evitar loop, usamos cart.length apenas como gatilho de adição/remoção
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        ...
        <p className="text-[#39404A] font-medium italic">Carregando cardápio de {apelido}...</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-[#39404A] mt-2">O link que você acessou pode estar incorreto ou o estabelecimento não existe.</p>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-5">
            ...
            >
              {cart.length > 0 ? (
                ...
                <ShoppingBasket className="w-9 h-9 text-[#39404A]/40" />
              )}
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-8 mt-2">
        ...
        <div className="relative group">
          <input 
            ...
            className="w-full bg-white border border-gray-200 rounded-[24px] px-8 py-5 text-sm shadow-sm focus:outline-none focus:ring-4 focus:ring-[#f45145]/5 transition-all placeholder:text-[#9CA3AF]"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <div className="absolute right-6 top-1/2 -translate-y-1/2 p-2 bg-gray-50 rounded-xl">
            <Search className="w-5 h-5 text-[#39404A] group-focus-within:text-[#f45145] transition-colors" />
          </div>
        </div>
                        ...
                        >
                            ...
                            <div className="w-full h-full relative rounded-[24px] overflow-hidden shadow-sm border border-gray-100">
                              {prod.foto ? (
                                ...
                                <div className="w-full h-full bg-gray-50 flex items-center justify-center group-hover:bg-orange-50 transition-colors">
                                  <ShoppingBasket className="w-16 h-16 text-[#39404A]/20 group-hover:text-orange-200 transition-colors" />
                                </div>
                              )}
                            </div>
                          </div>
                          <div className="p-6 text-center flex flex-col flex-1 justify-between gap-4">
                            <div className="space-y-2">
                              ...
                              <p className="text-xs text-[#39404A] font-medium leading-relaxed line-clamp-2 h-8">{prod.descricao || "Um sabor irresistível preparado com ingredientes selecionados."}</p>
                            </div>
                            <div className="pt-2">
              ...
              <div className="overflow-y-auto flex-1 custom-scrollbar">
                <div className="h-64 sm:h-72 relative">
                  {selectedProd.foto ? (
                    ...
                    <div className="w-full h-full bg-gray-50 flex items-center justify-center"><ShoppingBasket className="w-16 h-16 text-[#39404A]/20" /></div>
                  )}
                ...
                <div className="px-6 sm:px-10 -mt-10 relative z-10 space-y-6">
                  <div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-100">
                    ...
                    <p className="text-sm text-[#39404A] mt-2 font-medium leading-relaxed">{selectedProd.descricao || "Preparado com os melhores ingredientes da casa."}</p>
                    <div className="flex items-center gap-2 mt-4">
                      <span className="text-xs font-bold text-[#39404A] uppercase tracking-widest">A partir de</span>
                      ...
                      <div key={comp.id_complemento_tipo} className="space-y-3">
                        <div className="px-4 py-1 flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-[10px] text-[#39404A] uppercase tracking-widest mt-0.5 font-bold">
                              Escolha {comp.quantidade_minima || 0} a {comp.quantidade_maxima || 10} opções
                            </p>
                          </div>
                          <div className="flex gap-2">
                             <span className="bg-gray-100 text-[#39404A] text-[10px] px-2.5 py-1 rounded-lg font-black tracking-tighter uppercase italic">
                              ...
                              <div key={item.id_complemento_item} className="flex items-center justify-between p-5 rounded-3xl border border-gray-50 hover:bg-gray-50/30 transition-all">
                                ...
                                <div className="flex items-center gap-3 bg-white rounded-2xl p-1 shadow-sm border border-gray-100">
                                   <button 
                                     ...
                                     className={`p-2 rounded-xl transition-all active:scale-90 ${qty > 0 ? "bg-red-50 text-red-500 hover:bg-red-100" : "bg-gray-100 text-[#39404A]"}`}
                                   >
                                     <Minus className="w-4 h-4" />
                                   </button>
                                   <span className={`text-sm font-black w-4 text-center ${qty > 0 ? "text-gray-800" : "text-[#39404A]"}`}>{qty}</span>
                                   <button 
                                     onClick={() => handleItemQuantity(item.id_complemento_item, comp, 1)}
                  ...
                  <div className="pt-2 space-y-3">
                    <h4 className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest ml-4">Alguma observação?</h4>
                    <textarea 
                      placeholder="Ex: Sem cebola, retirar tomate..." 
                      className="w-full bg-gray-50 border border-gray-100 rounded-[32px] p-6 text-sm focus:outline-none focus:ring-4 focus:ring-[#f45145]/5 min-h-[100px] transition-all resize-none placeholder:text-[#9CA3AF]"
                      value={observation}
                  ...
                  <div className="pt-6 pb-16 space-y-4">
                    <div className="flex items-center justify-between bg-gray-50 p-5 rounded-[32px] border border-gray-100">
                      <span className="text-xs font-bold text-[#9CA3AF] uppercase tracking-widest ml-3">Quantidade</span>
                      <div className="flex items-center gap-6 bg-white px-6 py-3 rounded-2xl shadow-sm border border-gray-100">
                        <button onClick={() => setMainQuantity(q => Math.max(1, q - 1))} className="text-[#39404A] hover:text-red-500 active:scale-90 transition-all"><Minus className="w-5 h-5" /></button>
                ...
                {data?.horarios?.length > 0 ? (
                  data.horarios.map((h: any) => (
                    <div key={h.id_horario_funcionamento} className="flex justify-between items-center border-b border-gray-100 pb-3 last:border-0 last:pb-0">
                      ...
                      <div className="text-sm font-black text-[#39404A] bg-gray-50 px-3 py-1 rounded-lg tracking-widest border border-gray-100">
                  ...
                  <p className="text-center text-[#39404A] text-sm py-4">Nenhum horário cadastrado.</p>
                )}
              </div>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center">
            ...
            >
              ...
              <div className="bg-[#f45145] p-6 text-white relative">
                ...
                <div className="text-center">
                  ...
                  {data && !data.status?.aberto && (
                    <div className="mt-2 bg-white/20 px-3 py-1 rounded-full border border-white/20 inline-flex items-center gap-2 animate-pulse">
                      <Clock className="w-3 h-3 text-white" />
                      <span className="text-[9px] font-bold uppercase tracking-widest">Fechado no momento</span>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div key={item.id_unico} className="bg-gray-50/50 rounded-[32px] p-5 border border-gray-100/50 space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        {item.itens.length > 0 && (
                          <p className="text-[10px] text-[#9CA3AF] font-bold mt-1 leading-relaxed">
                        ...
                        {item.observacao && (
                          <div className="mt-3 bg-white/50 p-3 rounded-2xl border border-dashed border-gray-200">
                             <p className="text-[10px] text-[#9CA3AF] font-medium italic">Obs: {item.observacao}</p>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                      <span className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest">Quantidade</span>
              ...
              <div className="p-8 bg-gray-50 border-t border-gray-100 space-y-6">
                ...
                <button 
                  onClick={() => data?.status?.aberto && router.push(`/catalogo/delivery/${apelido}/checkout`)}
                  disabled={!data?.status?.aberto}
                  className={`w-full ${data?.status?.aberto ? 'bg-[#f45145] hover:bg-[#d43f35] shadow-[#f45145]/30' : 'bg-gray-400 cursor-not-allowed opacity-50'} text-white font-black py-6 rounded-[32px] shadow-2xl flex justify-between px-10 items-center transition-all hover:scale-[1.02] active:scale-95 group border-b-4 border-black/10`}
                >
                  <span className="text-lg tracking-widest uppercase italic">
                    {data?.status?.aberto ? 'FECHAR PEDIDO' : 'LOJA FECHADA'}
                  </span>
                ...
                <button 
                  ...
                  className="w-full text-[10px] font-black text-[#9CA3AF] uppercase tracking-[0.3em] hover:text-gray-600 transition-colors py-2"
                >
                  Continuar Comprando
                </button>
              </div>
