...
import { 
  UserCircle, Plus, Trash2, Loader2, Search, Edit3, X, CheckCircle2, User, Phone, Mail, Fingerprint, Lock, Camera 
} from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-white/40">Gerencie a base de clientes da sua loja.</p>
        </div>
        <button
          ...
          {filtered.map((cliente) => (
            ...
            >
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-xl font-bold text-white leading-tight">{cliente.nome}</h3>
                  {cliente.apelido && <p className="text-sm text-blue-400/60 font-medium">@{cliente.apelido}</p>}
                </div>
                
                <div className="pt-4 space-y-2">
                  {cliente.celular && (
                    <div className="flex items-center gap-2 text-xs text-white/40">
                      <Phone className="w-3 h-3" /> {maskPhone(cliente.celular)}
                    </div>
                  )}
                  {cliente.email && (
                    <div className="flex items-center gap-2 text-xs text-white/40">
...
function ModalCliente({ open, onClose, onSave, editData }: {
  open: boolean;
  onClose: () => void;
  onSave: () => void;
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      ...
      >
        ...
        <div className="flex items-center gap-5 mb-10">
          ...
          <div>
            <h2 className="text-3xl font-bold text-white">{editData ? "Editar Perfil" : "Novo Cliente"}</h2>
            <p className="text-white/40">Preencha os dados do seu cliente.</p>
          </div>
        </div>
